...
function OpenLicenseShop()
	...
	if Config.AdvancedWeaponShop then
		...
		if not ownedLicenses['weapon_heavy'] then
			table.insert(elements, {label = ('%s - <span style="color: green;">%s</span>'):format(_U('license_heavy'), _U('shop_menu_item', ESX.Math.GroupDigits(Config.Prices.Heavy))), value = 'buy_license_heavy'})
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'license_shop_actions', {
		...
		elseif data.current.value == 'buy_license_sniper' then
			...
			IsInMainMenu = false
			menu.close()
		elseif data.current.value == 'buy_license_heavy' then
			TriggerServerEvent('esx_licenseshop:buyLicenseHeavy')
			IsInMainMenu = false
			menu.close()
